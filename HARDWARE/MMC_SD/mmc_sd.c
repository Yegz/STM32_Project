...
u8 SD_GetResponse(u8 Response)
{
    ...
    while ((p_dr_SPIReadWriteByte(0XFF)!=Response)&&Count)
        Count--;
...
u8 SD_WaitDataReady(void)
{
    ...
    do
    {
        r1 = p_dr_SPIReadWriteByte(0xFF)&0X1F;
        if(retry==0xfffe)
            return 1;
        retry++;
        switch (r1)
...
u8 SD_Init(void)
{
    ...
    {
        buff[0] = p_dr_SPIReadWriteByte(0xFF);  //should be 0x00
        buff[1] = p_dr_SPIReadWriteByte(0xFF);  //should be 0x00
        buff[2] = p_dr_SPIReadWriteByte(0xFF);  //should be 0x01
        buff[3] = p_dr_SPIReadWriteByte(0xFF);  //should be 0xAA
        Set_SD_CS;
        p_dr_SPIReadWriteByte(0xFF);//the next 8 clocks
        ...
        //if(buff[2]==0x01 && buff[3]==0xAA)
        {
            ...
            buff[0] = p_dr_SPIReadWriteByte(0xFF);
            buff[1] = p_dr_SPIReadWriteByte(0xFF);
            buff[2] = p_dr_SPIReadWriteByte(0xFF);
            buff[3] = p_dr_SPIReadWriteByte(0xFF);
...
u8 SD_ReceiveData(u8 *data, u16 len, u8 release)
{
    ...
    while(len--)
    {
        *data = p_dr_SPIReadWriteByte(0xFF);
        data++;
    }
...
u8 SD_WriteSingleBlock(u32 sector, const u8 *data)
{
    ...
    for(i=0; i<512; i++)
    {
        p_dr_SPIReadWriteByte(*data++);
    }
    p_dr_SPIReadWriteByte(0xff);
    p_dr_SPIReadWriteByte(0xff);

    r1 = p_dr_SPIReadWriteByte(0xff);
    if((r1&0x1F)!=0x05)
    {
        Set_SD_CS;
        return r1;
    ...
    retry = 0;
    while(!p_dr_SPIReadWriteByte(0xff))
    {
        retry++;
        if(retry>0xfffe)
        {
...
u8 SD_Read_Bytes(unsigned long address,unsigned char *buf,unsigned int offset,unsigned int bytes)
{
    ...
    for(; i<offset+bytes; i++)
        *buf++ = p_dr_SPIReadWriteByte(0xff);
    for(; i<512; i++) 
        p_dr_SPIReadWriteByte(0xff);
    p_dr_SPIReadWriteByte(0xff);
    p_dr_SPIReadWriteByte(0xff);
