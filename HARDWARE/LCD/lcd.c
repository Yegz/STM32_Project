...
{										   
	...
	LCD_RD_CLR;
	if(DeviceCode==0X8989)delay_us(2);				   
	LCD_RD_SET;
	t=DATAIN;  
	LCD_CS_SET; 
#else
...
{
	if(DeviceCode==0X9341)LCD_WR_REG(0x2C);
}	 
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	LCD_SetCursor(x,y);
	if(DeviceCode==0X9341)LCD_WR_REG(0X2E);
	else LCD_WR_REG(R34);      		 	
	GPIOB->CRL=0X88888888; 
	GPIOB->CRH=0X88888888;
	GPIOB->ODR=0XFFFF;    
	...
	GPIOB->CRH=0X33333333; 
	GPIOB->ODR=0XFFFF;    
	if(DeviceCode==0X4535||DeviceCode==0X4531||DeviceCode==0X8989||DeviceCode==0XB505)return r;	
	else if(DeviceCode==0X9341)return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));						
	else return LCD_BGR2RGB(r);																
}

void LCD_DisplayOn(void)
{	
	if(DeviceCode==0X9341)LCD_WR_REG(0X29);	
	else LCD_WriteReg(R7,0x0173); 			
}	 

void LCD_DisplayOff(void)
{	   
	if(DeviceCode==0X9341)LCD_WR_REG(0X28);
...
{  
	...
	}else if(DeviceCode==0x9325) //9325 Init
	{
